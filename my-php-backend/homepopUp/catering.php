...
try {
    ...
    // Get the CategoryID for "Catering Services" from servicecateg
    $categoryStmt = $pdo->prepare('
        SELECT CategoryID 
        FROM servicecateg 
        WHERE CategName = "Catering Services"');
    $categoryStmt->execute();
    $category = $categoryStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$category) {
        echo json_encode(['error' => 'Catering Services" not found']);
        exit;
    }

    // Get the ServicesID and ServiceName for all subcategories in "Catering Services"
    ...
    if (empty($serviceProviderIDs)) {
        echo json_encode(['message' => 'No service providers found under "Catering Services"']);
        exit;
    }
    ...
    if ($providers) {
        ...
        echo json_encode(['message' => 'No providers found under "Catering Services"']);
    }
