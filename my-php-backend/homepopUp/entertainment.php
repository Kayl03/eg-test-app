...
try {
    ...
    $categoryStmt = $pdo->prepare('
        SELECT CategoryID 
        FROM servicecateg 
        WHERE CategName = "Entertainment & Activities"');
    $categoryStmt->execute();
    $category = $categoryStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$category) {
        echo json_encode(['error' => 'Category "Entertainment & Activities" not found']);
        exit;
    }

    // Get the ServicesID and ServiceName for all subcategories in "Entertainment & Activities"
    ...
    if (empty($serviceProviderIDs)) {
        echo json_encode(['message' => 'No service providers found under "Entertainment & Activities"']);
        exit;
    }
    ...
    if ($providers) {
        ...
        echo json_encode(['message' => 'No providers found under "Entertainment & Activities"']);
    }
