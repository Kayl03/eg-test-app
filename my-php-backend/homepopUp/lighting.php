...
try {
    ...
    // Step 1: Get the CategoryID and name for "Event Lighting" from servicecateg
    $categoryStmt = $pdo->prepare('
        SELECT CategoryID, CategName 
        FROM servicecateg 
        WHERE CategName = "Event Lighting"');
    $categoryStmt->execute();
    $category = $categoryStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$category) {
        echo json_encode(['error' => 'Category "Event Lighting" not found']);
        exit;
    }

    // SQL query to get the top 5 most rated service providers in "Event Lighting"
    $stmt = $pdo->prepare("
        SELECT 
    ...
    if ($providers) {
        ...
        echo json_encode(['message' => 'No providers found under "Event Lighting"']);
    }
