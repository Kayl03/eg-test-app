...
try {
    ...
    $categoryStmt = $pdo->prepare('
        SELECT CategoryID 
        FROM servicecateg 
        WHERE CategName = "Graphics & Design"');
    $categoryStmt->execute();
    $category = $categoryStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$category) {
        echo json_encode(['error' => 'Category "Graphics & Design" not found']);
        exit;
    }

    // Get the ServicesID and ServiceName for all subcategories in "Graphics & Design"
    ...
    if (empty($serviceProviderIDs)) {
        echo json_encode(['message' => 'No service providers found under "Graphics & Design"']);
        exit;
    }
    ...
    if ($providers) {
        ...
        echo json_encode(['message' => 'No providers found under "Graphics & Design"']);
    }
