...
try {
    ...
    // Get the CategoryID for "Gowns & Suits" from servicecateg
    $categoryStmt = $pdo->prepare('
        SELECT CategoryID 
        FROM servicecateg 
        WHERE CategName = "Gowns & Suits"');
    $categoryStmt->execute();
    $category = $categoryStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$category) {
        echo json_encode(['error' => 'Gowns & Suits" not found']);
        exit;
    }

    // Get the ServicesID and ServiceName for all subcategories in "Gowns & Suits"
    ...
    if (empty($serviceProviderIDs)) {
        echo json_encode(['message' => 'No service providers found under "Gowns & Suits"']);
        exit;
    }
    ...
    if ($providers) {
        ...
        echo json_encode(['message' => 'No providers found under "Gowns & Suits"']);
    }
