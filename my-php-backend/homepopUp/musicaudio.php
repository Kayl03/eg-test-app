...
try {
    ...
    $categoryStmt = $pdo->prepare('
        SELECT CategoryID 
        FROM servicecateg 
        WHERE CategName = "Music & Audio"');
    $categoryStmt->execute();
    $category = $categoryStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$category) {
        echo json_encode(['error' => 'Category "Music & Audio" not found']);
        exit;
    }
    ...
    if (empty($serviceProviderIDs)) {
        echo json_encode(['message' => 'No service providers found under "Music & Audio"']);
        exit;
    }
    ...
    if ($providers) {
        ...
        echo json_encode(['message' => 'No providers found under "Music & Audio"']);
    }
